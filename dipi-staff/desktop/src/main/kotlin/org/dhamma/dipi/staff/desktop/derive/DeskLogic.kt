...
fun deskRoomSyncPending(checkIns: Map<ApplicantId, CheckInRecord>): Int =
    org.dhamma.dipi.staff.model.RoomAllocSync.pending(checkIns).size

fun roomPullSnack(n: Int) =
    if (n == 0) org.dhamma.dipi.staff.model.FlushSnack("No rooms assigned on the desk yet", error = false)
    else org.dhamma.dipi.staff.model.FlushSnack("✓ Pulled $n room assignment(s) from the desk", error = false)

fun roomSyncSnack(result: org.dhamma.dipi.staff.model.RoomSyncResult) = when {
    result.offline ->
        org.dhamma.dipi.staff.model.FlushSnack("${result.synced} synced · connection lost — the rest will sync when online", error = true)
    result.failures.isNotEmpty() ->
        org.dhamma.dipi.staff.model.FlushSnack("${result.synced} synced · ${result.failed} failed — ${result.failures.first().reason}", error = true)
    else -> org.dhamma.dipi.staff.model.FlushSnack("✓ Synced ${result.synced} room allocation(s) to the desk", error = false)
}
